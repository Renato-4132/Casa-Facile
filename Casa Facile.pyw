...
class GestioneSpese(tk.Tk):
    ...
    def __init__(self):
        ...
        menu_webserver = tk.Menu(barra_menu, tearoff=0, bg=MENU_BG, fg=FOREGROUND_LIGHT, activebackground=ACCENT_COLOR, activeforeground='black')
        ...
        menu_db.add_command(label="📥 Importa Consumi", command=self.import_db)
        menu_db.add_command(label="📤 Esporta Consumi", command=self.export_db)
        menu_db.add_command(label="📤 Reset DB", command=self.show_reset_dialog)
        menu_db.add_command(label="📤 Esegui Backup", command=self.esegui_backup_zip)
        ...
        self.after(2000, self.check_supermarket_update)
    ...
    def mostra_help(self):
        ...
        popup_aiuto.title("❓ Aiuto: Filtri, Tabelle e Grafici")
    ...
    def show_info_app(self):
        ...
        btn_aggiorna = ttk.Button(info_win, text="🔄 Aggiorna Software", command=chiudi_e_aggiorna, style='Verde.TButton')
    ...
    def utenze(self):
        ...
        menu_funzioni.add_command(label="❌ Chiudi", command=lambda: (self.deiconify(), win.destroy()))
  ...
  • Spesa effettiva {anno_prec}: € {stima_anno_prec:,.2f}
    ...
    def spesa_supermercato(self):
        ...
        def _mostra_anteprima_esportazione():
            ...
            ttk.Button(frame_btn, text="Esporta", style='Verde.TButton',
                        command=lambda: _esporta_su_file(anteprima_text, preview_popup)).pack(side='right', padx=5)
            ttk.Button(frame_btn, text="🖨️ Stampa ora", style='Arancio.TButton', command=lambda: self._stampa_lista_diretta(anteprima_text, self.show_custom_warning)).pack(side='left', padx=5)
        ...
        menu_db.add_command(label="⬇️ Scarica Editor Scontrini", command=self._scarica_editor_esterno)
        menu_db.add_command(label="❌ Chiudi", command=lambda: (self.deiconify(), popup.destroy()))
    ...
    def mostra_help_supermercati(self):
        ...
        def _crea_text_area(parent_frame, content):
            ...
            text_area.tag_configure('indent', lmargin1=15, lmargin2=30) 
            text_area.insert('1.0', content)
            text_area.tag_add('indent', '1.0', 'end') 
            text_area.config(state='disabled') 
...
3.  **Aggiunta Articoli e Accumulo Quantità (Doppio Click):**
    ...
    * **Cruciale:** Se l'articolo è **già presente** nella lista inferiore, un ulteriore doppio click su di esso non aggiunge una nuova riga, ma **incrementa automaticamente la quantità da comprare di 1 unità**.

### B. Sezione Lista Spesa Ottimizzata (Tabella Inferiore)
1.  **Ottimizzazione Prezzo/Supermercato:** La lista spesa finale suggerisce gli acquisti. La colonna **'Supermercato'** indica dove è consigliato comprare l'articolo per ottenere il prezzo finale più basso.
...
3.  **✏️ Modifica (Carica):** Seleziona un articolo nella tabella e premi 'Modifica' per caricare i suoi dati nei campi di input per l'editing.
...
* **➡️ Sposta Articolo:** Sposta l'articolo selezionato dal catalogo corrente a un altro supermercato, specificato nel ComboBox di destinazione.
* **✏️ Rinomina Selezionato:** Permette di cambiare il nome del supermercato selezionato.
* **💣 AZZERA Dati:** **Attenzione!** Cancella **TUTTI** gli articoli del supermercato selezionato.
* **🖨️ Esporta Catalogo:** Salva l'intero catalogo del supermercato selezionato in un file di testo per backup o consultazione esterna.
...
"""
        ...
        btn_frame = ttk.Frame(help_popup)
        btn_frame.pack(fill='x', padx=15, pady=(5, 15))
        ttk.Button(btn_frame, text="Ho Capito (OK)", command=help_popup.destroy, style='Giallo.TButton').pack(
            fill='none', 
            anchor='center'
        )
    ...
    def gestisci_archivi_pdf(self, categoria_iniziale=None):
        ...
        menu_archivio.add_command(label="❌ Chiudi", command=chiudi_finestra)
    ...
    def mostra_help_pdf(self):
        if hasattr(self, '_filter_help_popup') and self._filter_help_popup.winfo_exists():
            self._filter_help_popup.destroy()
        popup_aiuto = tk.Toplevel(self)
        popup_aiuto.title("❓ Aiuto: Archivio Documenti PDF")
