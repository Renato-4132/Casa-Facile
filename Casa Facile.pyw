    ...
    def spesa_supermercato(self):
        ...
        def _funzione_crud(azione, supermercato, treeview, frame_input):
            ...
            if azione == 'inserisci':
                ...
                if DateEntry:
                    try:
                        data_dt = datetime.datetime.strptime(data_scadenza_str, '%d/%m/%Y').date()
                        date_entry_widget = frame_input.grid_slaves(row=3, column=3)
                        if date_entry_widget and isinstance(date_entry_widget[0], DateEntry):
                            date_entry_widget[0].set_date(data_dt)
                    except (ValueError, IndexError):
                        pass
        ...
        def info_popup():
            ...
            messaggio = (
                ...
                "viene primariamente popolato tramite l'importazione degli scontrini digitali.\n\n"

                "Su richiesta, è disponibile un Plugin separato che analizza gli scontrini, "
                "estrae i nomi degli articoli, le categorie e i prezzi, e li archivia "
                "in automatico per permettere la consultazione e il confronto."
            )
            frame_contenuto = ttk.Frame(popup, padding="20")
        ...
        risultati_tv.heading("Nome Articolo", text="Articolo"); risultati_tv.heading("Qtà Catalogo", text="Qtà Catalogo")
        risultati_tv.heading("Supermercato", text="Supermercato"); risultati_tv.heading("Prezzo", text="Prezzo")
        ...
        for col in cols_risultati:
            risultati_tv.heading(col, text=col.replace(" ", "\n"), command=lambda _col=col, t=risultati_tv: _sort_treeview(t, _col, False))
        ...
        for col in cols_lista_spesa: tv_lista_spesa.heading(col, text=col.replace(" ", "\n"))
        ...
        if DateEntry:
            DateEntry(frame_input, textvariable=refs_crud['data_scadenza'],
                      date_pattern='dd/mm/yyyy', width=12, background='darkblue',
                      foreground='white', borderwidth=2).grid(row=3, column=3, padx=5, pady=2, sticky='ew')
        else:
            ttk.Entry(frame_input, textvariable=refs_crud['data_scadenza']).grid(row=3, column=3, padx=5, pady=2, sticky='ew')
        ...
        ttk.Button(frame_pulsanti_crud, text="💣 AZZERA Database Supermercato", style='Rosso.TButton',
