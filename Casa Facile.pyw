...
# Attiva/disattiva Timer Iconizza
ICONIZZA_INATTIVITA = True

# 20 minuti in millisecondi Timer Iconizza
...
BACKGROUND_DARK = "black"  # Barra menu superiore
FOREGROUND_LIGHT = "white"
MENU_BG = "#424242"        # Sfondo sottomenu
...
class GestioneSpese(tk.Tk):
    ...
    def __init__(self):
        ...
        menu_gestione.add_command(label="📋 Gestione SuperMarket", command=self.spesa_supermercato)
        menu_gestione.add_command(label="📤 Gestione Documenti", command=self.gestisci_archivi_pdf)
        ...
        menu_ricorrenze = tk.Menu(barra_menu, tearoff=0, bg=MENU_BG, fg=FOREGROUND_LIGHT, activebackground=ACCENT_COLOR, activeforeground='black')
        ...
        menu_analisi.add_command(label="📊 Time Machine", accelerator="Ctrl+W", command=self.time_machine)
        ...
        menu_analisi.add_command(label="📋 Report", accelerator="Ctrl+A", command=self.calcola_statistiche_annuali)
        ...
        menu_estrazioni = tk.Menu(barra_menu, tearoff=0, bg=MENU_BG, fg=FOREGROUND_LIGHT, activebackground=ACCENT_COLOR, activeforeground='black')
        ...
        barra_menu.add_cascade(label="⚙️ Opzioni", menu=menu_opzioni)
        menu_opzioni.add_command(label="🖥️ Visualizza Interfaccia Web", command=self.apri_webserver)
        menu_opzioni.add_separator()
        menu_opzioni.add_command(label="⚙️ Impostazioni App", command=self.gestisci_configurazione)
        menu_opzioni.add_separator()
        menu_opzioni.add_command(label="🔄 Controlla Aggiornamento Software", command=self.forza_check_aggiornamento_con_api)
        menu_opzioni.add_command(label="🔄 Forza Aggiornamento Software", command=self.forza_aggiorna)
        menu_opzioni.add_separator()
        menu_opzioni.add_command(label="↩️ Annulla Ultimo Aggiornamento", command=self.ripristina_da_backup)
        menu_opzioni.add_separator()
        menu_opzioni.add_command(label="📄 Visualizza Storico Aggiornamenti", command=self.visualizza_changelog)
        menu_db = tk.Menu(barra_menu, tearoff=0, bg=MENU_BG, fg=FOREGROUND_LIGHT, activebackground=ACCENT_COLOR, activeforeground='black')
        barra_menu.add_cascade(label="💾 Database", menu=menu_db)
        menu_db.add_command(label="📤 Esporta DB Transazioni", command=self.export_db)
        menu_db.add_command(label="📤 Importa DB Transazioni", command=self.import_db)
        menu_db.add_command(label="📤 Reset DB", command=self.show_reset_dialog)
        menu_db.add_command(label="📤 Esegui Backup Completo", command=self.esegui_backup_zip)
        ...
        style.configure("Saldo.TLabel", foreground="green", font=("Arial", 14))
        style.configure("Doc.TLabel", foreground="green", font=("Arial", 14))
        style.configure("Legend.TLabel", font=("Arial", 10),anchor="w")
        ...
        self.btn_analisi = ttk.Button(mode_frame, text="🔍", command=lambda: self.mostra_analisi_grafici(), width=2, style="Blu.TButton")
        ...
        self.btn_giorno = ttk.Button(mode_frame, text="📅 Giorno", command=lambda: self.set_stats_mode("giorno"), width=9, style="Blu.TButton")
        self.btn_giorno.pack(side=tk.LEFT, padx=1)
        self.btn_mese = ttk.Button(mode_frame, text="📅 Mese", command=lambda: self.set_stats_mode("mese"), width=9, style="Blu.TButton")
        self.btn_mese.pack(side=tk.LEFT, padx=1)
        self.btn_anno = ttk.Button(mode_frame, text="📅 Anno", command=lambda: self.set_stats_mode("anno"), width=9, style="Blu.TButton")
        self.btn_anno.pack(side=tk.LEFT, padx=1)
        self.btn_totali = ttk.Button(mode_frame, text="📅 Totali", command=lambda: self.set_stats_mode("totali"), width=9, style="Blu.TButton")
        ...
        lbl_saldo_tot = ttk.Label(mode_frame, text="💰", style="Saldo.TLabel")
        ...
        self.stats_text_area = tk.Text(table_container, wrap=tk.WORD, state=tk.DISABLED, bg="#F0F0F0")
        headers = {
            "A": "🗓️ Data",
            "B": "📂 Categoria",
            "C": "📝 Descrizione",
    ...
    def toggle_stats_view(self, tipo="grafico"):
        ...
        if tipo in ("grafico", "grafico_mensile", "grafico_saldo"):
            ...
            self.stats_table = ttk.Treeview(parent_frame, columns=("A", "B", "C", "D", "E", "F"), show="headings", style="Stat.Custom.Treeview")
            headers = {
                "A": "🗓️ Data", "B": "📂 Categoria", "C": "📝 Descrizione",
                "D": "💰 Importo (€)", "E": "📌 Tipo", "F": "✏️ Modifica"
            }
            for col in headers:
                self.stats_table.heading(col, text=headers[col], command=lambda _col=col: self.treeview_sort_column(self.stats_table, _col, False))
            self.stats_table.column("A", width=100, anchor="center")
    ...
    def draw_bar_chart(self, event=None):
        ...
        btn_text = "🗓 Tutti gli anni" if not self.visualizza_tutti_gli_anni else "📅 Solo anno corrente"
        ...
        if self.visualizza_tutti_gli_anni:
            ...
            titolo = f"📊 Uscite per Categoria (Tutto lo Storico) {periodo}"
        else:
            periodo = f"({anno_corrente})"
            titolo = f"📊 Uscite per Categoria - Anno Corrente {periodo}"
        ...
        for i, item in enumerate(self.data_for_chart):
            ...
            canvas.create_text(x_center, y1 - 5, text=f"{item['value']:.0f}", anchor="s",
                               font=("Arial", 7, "bold"), fill=color)
            tooltip_text = f"{item['label']}: €{item['value']:.2f}"
            ...
            canvas.tag_bind(tag, "<Double-1>", lambda e, f=data_filter, l=item['label']:
                            self.mostra_transazioni_popup(f, f"Uscite per {l}"))
        ...
        messaggio_riepilogo = (
            f"Totale Visualizzato: {self.total_value:,.2f} €  |  "
            f"Totale Anno: {self.total_annuale:,.2f} €  |  "
    ...
    def draw_mensile_chart(self, event=None):
        ...
        for i, item in enumerate(self.data_for_chart):
            ...
            h_entrata = (item["entrata"] / max_value) * CHART_HEIGHT
            h_uscita = (item["uscita"] / max_value) * CHART_HEIGHT
            ...
            tooltip_entrata = f"{item['label']} - Entrata: €{item['entrata']:.2f}"
            tooltip_uscita = f"{item['label']} - Uscita: €{item['uscita']:.2f}"
            ...
            canvas.tag_bind(tag_entrata, "<Double-1>", lambda e, f=data_filter_e, l=item["label"]: self.mostra_transazioni_popup(f, f"Entrate {l}"))
            canvas.tag_bind(tag_uscita, "<Double-1>", lambda e, f=data_filter_u, l=item["label"]: self.mostra_transazioni_popup(f, f"Uscite {l}"))
        if self.visualizza_tutti_gli_anni_mensile:
        ...
        x_pos_2 = CHART_LEFT + CHART_AREA_WIDTH * 0.
    ...
    def show_tooltip(self, event, text):
        ...
        label = tk.Label(self.tooltip_window, text=text, background="#FFFFEA",
    ...
    def draw_saldo_chart(self, event=None):
        ...
        if self.visualizza_saldo_10_anni:
            ...
            canvas.create_text(CHART_LEFT - 5, scale_y(max_saldo), text=f"€{max_saldo:,.0f}", anchor="e", font=("Arial", 8), fill="black", tags="y_labels")
            canvas.create_text(CHART_LEFT - 5, scale_y(min_saldo), text=f"€{min_saldo:,.0f}", anchor="e", font=("Arial", 8), fill="black", tags="y_labels")
            if y_min < 0 < y_max:
            ...
            for i, item in enumerate(self.data_for_chart):
                ...
                if i == 0 or (item['data'].month == 1 and item['data'].day == 1):
            ...
            if data_max.year != data_min.year:
                canvas.create_text(CHART_RIGHT, CHART_BOTTOM + NUOVO_OFFSET_TESTO, text=data_max.strftime("%Y"), anchor="n", font=("Arial", 8))
        else:
            max_positive = max((item["saldo"] for item in self.data_for_chart if item["saldo"] >= 0), default=0)
            max_negative = min((item["saldo"] for item in self.data_for_chart if item["saldo"] < 0), default=0)
        ...
        for giorno, entries in self.spese.items():
            ...
            if self.visualizza_saldo_10_anni and data_min_vis and data_max_vis:
                 if giorno >= data_min_vis and giorno <= data_max_vis:
                     include_entry = True
    ...
    def gestisci_promemoria(self):
        ...
        btn_esporta_promemoria = ttk.Button(center_btn_frame, text="📄 Esporta", command=esporta_promemoria, style="Arancio.TButton")
        ...
        try:
            if sys.platform != "win" and _lock_file_handle:
                ...
                if os.path.exists(lock_file_path):
                    ...
                    print("🗑️ File di lock cancellato.")
        except Exception as e:
            print(f"⚠️ Errore durante la pulizia del lock file: {e}")
    ...
    def mostra_lista_ricorrenze(self):
        ...
        def destroy_window_and_cleanup():
            if hasattr(self, 'lista_window_ref'):
        ...
        lista_window.lift()
        lista_window.bind("<Escape>", lambda e: destroy_window_and_cleanup())
        lista_window.protocol("WM_DELETE_WINDOW", destroy_window_and_cleanup)
    ...
    def show_reset_dialog(self):
        ...
        def do_yes():           
            try:
                ...
                try:
                    ...
                    pass 
                show_custom_warning("Spese", "Spese e categorie azzerate")
            except Exception as e:
                traceback.print_exc()
        def do_no():
            dialog.destroy()
            ...
            self.update_totalizzatore_mese_corrente()
            self.update_spese_mese_corrente()
            show_custom_warning("Spese", "Spese azzerate")
        def do_letture():
            dialog.destroy()
            ...
            if not os.path.exists(UTENZE_DB):
               with open(UTENZE_DB, "w") as file:
                 file.write("{\n}\n")  
                 show_custom_warning("Letture", "Letture utenze azzerate")
        def do_rubrica():
            dialog.destroy()
            if os.path.exists(DATI_FILE):
             os.remove(DATI_FILE)
            show_custom_warning("Rubrica", "Dati Rubrica azzerate")
        def do_password():
            dialog.destroy()
        ...
        ttk.Button(btns, text="❌ RESET", command=do_reset, style="Giallo.TButton").grid(row=0, column=5, padx=5)
        ttk.Button(btns, text="❌ Annulla", command=do_cancel, style="Giallo.TButton").grid(row=0, column=6, padx=5)
    ...
    def show_export_preview(self, content, default_filename=None):
        ...
        btn_salva = ttk.Button(btn_frame, text="💾 Esporta", command=save_file, style="Verde.TButton")
        btn_salva.pack(side=tk.LEFT, padx=10)

        btn_stampa = ttk.Button(
            btn_frame, 
            text="🖨️ Stampa", command=lambda: self._stampa_lista_diretta(content, self.show_custom_warning),
            style="Blu.TButton"
        )
    ...
    def open_analisi_categoria(self):
        ...
        def esporta_analisi():
            ...
            ttk.Button(frm, text="🖨️ Stampa", style="Blu.TButton", command=lambda: self._stampa_lista_diretta(
    ...
    def open_saldo_conto(self):
        ...
        if hasattr(self, 'spese') and self.spese:
            for giorno in self.spese.keys():
                if hasattr(giorno, "year"):
                    anni_spese.add(giorno.year)
        anni_spese.add(anno_corrente)
        if anni_spese:
            anni = sorted(list(anni_spese))
        else:
        ...
        ).grid(row=1, column=1, columnspan=2, sticky="w", padx=4, pady=(14,2))
        ...
        def aggiorna_saldi(*_):
            lbl_saldo_mese.config(text=f"Saldo nel mese: {calcola_saldo_popup('mese'):.2f} €")
            lbl_saldo_anno.config(text=f"Saldo nell'anno: {calcola_saldo_popup('anno'):.2f} €")
            lbl_saldo_tot.config(text=f"Saldo totale    : {calcola_saldo_popup('totale'):.2f} €")
    ...
    def show_tooltip_saldo(self, event):
        ...
        tooltip_text = (
            ...
            f"{'Saldo iniziale:':<18} € {self.saldo_iniziale:.2f}\n"
            f"{'Data saldo:':<18}    {self.saldo_data.strftime('%d-%m-%Y')}\n"
            f"{'Saldo attuale:':<18} € {self.saldo_attuale:.2f}\n"
            f"{'Saldo previsto:':<18} € {self.saldo_previsto:.2f}"
        )
    ...
    def open_compare_window(self):
        ...
        def do_preview_export():
            ...
            frm = ttk.Frame(prev)
            frm.pack(fill=tk.X, padx=10, pady=8)
            ttk.Button(frm, text="💾 Esporta", command=do_save, style="Verde.TButton").pack(side=tk.LEFT, padx=6)
            ttk.Button(frm, text="🖨️ Stampa", command=lambda: self._stampa_lista_diretta(text, self.show_custom_warning), style="Blu.TButton").pack(side=tk.LEFT, padx=6)
        ...
        ttk.Button(btnframe, text="📄 Esporta", command=do_preview_export, style="Verde.TButton").pack(side=tk.LEFT, padx=8)
    ...
    def scadenze_mese(self):
        ...
        ttk.Button(button_frame, text="❌ Chiudi", command=on_scadenze_close, style='Giallo.TButton').pack(side="right", padx=20, pady=5)
    ...
    def forza_aggiorna(self):
        ...
        if risposta:
            ...
            self.show_custom_warning("Aggiornamento Annullato", "❌ Aggiornamento forzato annullato dall'utente.")
    ...
    def calcolo_mutuo_prestito(self):
        ...
        def esporta_dati_con_preview():
            ...
            risultati_principali = self.tutti_i_risultati[0] if self.tutti_i_risultati and self.tutti_i_risultati[0] else None
            for i, res in enumerate(self.tutti_i_risultati):
                if res:
                    ...
                    risparmio = 0.0
                    if risultati_principali and i > 0:
                        risparmio = risultati_principali["interessi_totali"] - res["interessi_totali"]
        ...
        root.protocol("WM_DELETE_WINDOW", lambda: (self.deiconify(), root.destroy()))
        root.bind("<Escape>", lambda e: (self.deiconify(), root.destroy()))
        self.iconify() 
        self.tutti_i_risultati = [None] * 6
        ...
        ttk.Button(common_button_frame, text="❌ Chiudi", command=lambda: (self.deiconify(), common_button_frame.winfo_toplevel().destroy()), 
    ...
    def on_stats_table_right_click(self, event):
        try:
            ...
            print(f"Errore durante l'elaborazione del click destro: {e}")
    ...
    def crea_grafico_categorie(self, id_righe_selezionate):
        ...
        if stats_mode == "giorno" and stats_refdate: 
            ...
            filtro_titolo = f" - Mese {mese_filtro}/{anno_filtro}"
            totale_copertura_desc = f"{totale_copertura_desc_saldo} Mese {mese_filtro} / {anno_filtro}"
            ...
            etichetta_tempo = "Mensili" 
            filtro_titolo = " - Totale"
            totale_copertura_desc = f"{totale_copertura_desc_saldo} Totale"
        ...
        popup_grafico.title(f"📊 {titolo_grafico}")
        screen_width = self.winfo_screenwidth()
        screen_height = self.winfo_screenheight()
        ...
        main_frame = ttk.Frame(popup_grafico)
        main_frame.pack(fill="both", expand=True, padx=10, pady=10)
        tk.Label(main_frame, text=titolo_grafico, font=("Arial", 12, "bold")).pack(pady=(0, 5))
        saldo_color = "green" if saldo_netto_periodo >= 0 else "red"
        riepilogo_testo = (
            f"💰 Totale Entrate: {totale_entrate_periodo:,.2f} € "
            f"| 💸 Totale Uscite: {totale_uscite_periodo:,.2f} € "
            f"| Saldo Netto: {saldo_netto_periodo:,.2f} €"
        )
        lbl_riepilogo = tk.Label(
            main_frame, 
            text=riepilogo_testo, 
            font=("Arial", 10, "bold"),
            foreground=saldo_color 
        )
        lbl_riepilogo.pack(pady=(0, 15)) 
        ...
        canvas.bind("<Configure>", lambda e: self.disegna_barre_scroll(canvas, dati_per_grafico))
        ...
        def disegna_barre_scroll(c, dati_tuple):
            ...
            tooltip_window_local = None

            def hide_tooltip_local(event=None):
                nonlocal tooltip_window_local
                if tooltip_window_local:
                    tooltip_window_local.destroy()
                    tooltip_window_local = None

            def show_tooltip_local(event, text):
                nonlocal tooltip_window_local
                hide_tooltip_local() 
                root_x = c.winfo_rootx()
                root_y = c.winfo_rooty()
                screen_x = root_x + event.x + 10
                screen_y = root_y + event.y + 10
                tooltip_window_local = tk.Toplevel(c)
                tooltip_window_local.wm_overrideredirect(True)
                tooltip_window_local.wm_geometry(f"+{screen_x}+{screen_y}")
                label = tk.Label(tooltip_window_local, 
                                 text=text, 
                                 background="#FFFFEA", 
                                 relief="solid", 
                                 borderwidth=1,
            ...
            for _, val in dati_tuple:
                if isinstance(val, dict):
                    if 'Totale' in val:
                         ...
                         importi_entrate.extend([val.get('Entrata', 0), 0])
                         importi_uscite.extend([val.get('Uscita', 0), 0])
                else:
                    if tipo_transazione_ricercato == "entrata":
            ...
            for periodo, dati_valore in dati_tuple:
                ...
                if stats_mode == "giorno":
                    larghezza_gruppo = LARGHEZZ_STANDARD
                else:
                    larghezza_gruppo = barra_width
            ...
            if stats_mode == "giorno":
                larghezza_contenuto = canvas_larghezza
            else:
                larghezza_contenuto = max(larghezza_contenuto_barre, canvas_larghezza + SPAZIO_EXTRA_MINIMO) 
                ...
                if is_misto:
                    ...
                    if valore_e > 0:
                        ...
                        tooltip_text_e = (
                            ...
                            f"\n"
                            #f"--- Dettaglio Categorie Selezionate ---\n"
                            f"{dettaglio_cat_e}"
                        ...
                        filter_data = self.get_filter_data(periodo, 'Entrata', categorie_da_elaborare, stats_mode)
                        c.tag_bind(rect_e, "<Double-1>", lambda e, f=filter_data: self.mostra_transazioni_popup(f, f"Entrate aggregate {periodo}"))
                    if valore_u > 0:
                        ...
                        filter_data = self.get_filter_data(periodo, 'Uscita', categorie_da_elaborare, stats_mode)
                        c.tag_bind(rect_u, "<Double-1>", lambda e, f=filter_data: self.mostra_transazioni_popup(f, f"Uscite aggregate {periodo}"))
                    ...
                    filter_data = self.get_filter_data(periodo, tipo_bind, categorie_da_elaborare, stats_mode)
                    c.tag_bind(rect, "<Double-1>", lambda e, f=filter_data: self.mostra_transazioni_popup(f, f"{tipo_transazione_base} aggregate {periodo}"))
        ...
        popup_grafico.update_idletasks()
        disegna_barre_scroll(canvas, dati_per_grafico)
        popup_grafico.after(3000, esegui_disegno_ritardato) 
        popup_grafico.transient(self)
        if stats_mode != "giorno":
            popup_grafico.after(10, lambda: canvas.xview_moveto(0)) 
        ...
        if tipo_transazione_ricercato == "misto":
            ...
            colore_totale = "#DC3545"
        totale_frame = ttk.Frame(main_frame)
        ...
        popup_grafico.bind("<Escape>", lambda event: chiudi_popup())
    ...
    def mostra_analisi_grafici(self):
        def disegna_barre(canvas, dati, colori, mostra_anno=False, mostra_tipo=False, centro=False):
            ...
            for i, (etichetta, valore) in enumerate(elementi):
                ...
                if valore >= 0:
                    ...
                    canvas.create_text((x0 + x1) / 2, y1 - 10, text=f"{valore:,.2f}", font=("Arial", 8))
                else:
                ...
                if mostra_tipo and " " in etichetta:
                    ...
                    canvas.create_text((x0 + x1) / 2, y_base + 10, text=tipo_label, font=("Arial", 8))
        ...
        def mostra_legenda_categorie(canvas_master, categorie_dati, colori):
            ...
            legenda_frame.grid(row=1, column=1, sticky="ns", padx=10, pady=(0, 10))
        ...
        def aggiorna_tab2(event=None):
            ...
            disegna_barre_categorie(canvas2, tutte_categorie, colore)
            mostra_legenda_categorie(tab2, tutte_categorie, colore)
        ...
        tab2.grid_columnconfigure(0, weight=99) 
        ...
        canvas2.bind("<Configure>", lambda e: disegna_barre_categorie(canvas2, canvas2.tutte_categorie, canvas2.colori))
        aggiorna_tab2() 
    ...
    def mostra_transazioni_popup(self, data_filter, title):
        ...
        categoria_filtro = data_filter.get("categoria") 
        tipo_filtro = data_filter.get("tipo")
        if categoria_filtro and categoria_filtro != "Tutti":
            categoria_filtro = categoria_filtro.strip().title()
        spese_filtrate = []
        for data, voci in self.spese.items():
            if anno and anno != "Tutti" and str(data.year) != anno:
                continue
            if mese and mese != "Tutti" and data.month != mese:
                continue
            for entry in voci:
                try:
                    cat_originale, desc, imp_str, entry_tipo = entry[:4]
                    cat_normalized = cat_originale.strip().title() 
                    entry_imp = float(imp_str)
                    entry_tipo = entry_tipo.strip()
                except (ValueError, TypeError, IndexError): 
                    continue 
                if categoria_filtro and cat_normalized != categoria_filtro:
                    continue
                if tipo_filtro and entry_tipo != tipo_filtro:
                    continue
                spese_filtrate.append((data, cat_normalized, desc, entry_imp, entry_tipo))
    ...
    def mostra_help(self):
        ...
        popup_aiuto = tk.Toplevel(self)
        popup_aiuto.title("Guida: Filtri, Tabelle e Grafici")
        popup_aiuto.geometry("1200x570")
        ...
        help_content = [
            ...
            ("Giorno (Punto Singolo)", "Mostra le spese solo per il **GIORNO** selezionato."),
        ]
        ...
        tk.Label(
            content_frame,
            text="• **Scroll e Navigazione:** Utilizza la **Rotella del Mouse** per scorrere verticalmente in qualsiasi Treeview o area.\n"
                 "• **Ordinamento (Sort):** Clicca sull'intestazione di qualsiasi colonna per ordinare i dati. Clicca di nuovo per invertire (crescente/decrescente).\n"
                 "• **Selezione Righe (CTRL/SHIFT):** Utilizza **CTRL + Click** per selezionare righe sparse (selezione mista), o **SHIFT + Click** per selezionare un intervallo continuo di righe.",
            font=("Arial", 9),
            justify=tk.LEFT,
            anchor='w'
        ...
        tk.Label(
            content_frame,
            text="• **Dettaglio Transazioni:**\n"
                 "  1. Doppio Clic sulla riga (Treeview Statistiche): Apre il pop-up con il **Dettaglio** di tutte le transazioni aggregate.\n"
                 "  2. Doppio Clic sulla riga (Popup Dettaglio): **Seleziona la riga** della transazione e ti reindirizza automaticamente alla transazione corrispondente nella Treeview principale.",
            font=("Arial", 9),
            justify=tk.LEFT,
            anchor='w'
        ).pack(fill='x', padx=5, pady=(0, 5))
        ...
        tk.Label(
            content_frame,
            text="• **Navigazione Grafici:** Usa le **Frecce Destra/Sinistra** per scorrere tra le diverse visualizzazioni (Tabella, Grafico, Mensile, Saldo). Premi **ESC** per tornare alla Treeview.\n"
                 "• **Avvio Archivio PDF:** Clicca con il **Tasto Destro** su una riga (spesa o entrata) per avviare l'Archivio Documenti, pre-filtrato per la categoria selezionata.\n"
                 "• **Avvio Grafico Multiplo:** Dopo aver selezionato una o più categorie dalla Treeview (usando **CTRL/SHIFT**), clicca con il **Tasto Destro** del mouse sulle righe selezionate per avviare il grafico.\n"
                 "• **Dettaglio Grafico (Tooltip):** Passa il mouse sopra una barra per visualizzare un **Tooltip** che mostra il totale e il **dettaglio delle singole categorie** aggregate in quella barra.\n"
                 "• **Dettaglio Spesa (Drill-Down):** Doppio Clic su una barra del grafico per aprire il dettaglio delle transazioni relative a quel periodo (Giorno/Mese).\n"
    ...
    def show_info_app(self):
        ...
        text.insert("end", "• Analisi: Giornaliere, Mensili, Annuali, Totali, Grafici, Time Machine, Confronta, Documenti, Supermarket.\n")
        ...
        btn_stampa.pack(side=tk.LEFT, padx=5)
        btn_chiudi = ttk.Button(button_frame, text="❌ Chiudi", command=info_win.destroy, style='Giallo.TButton')
    ...
    def utenze(self):
        ...
        menu_funzioni.add_command(label="❌ Chiudi (ESC)", command=lambda: (self.deiconify(), win.destroy()))
        ...
        win.bind("<Escape>", lambda e: (self.deiconify(), win.destroy()))
        ...
        def esporta_preview():
            ...
            ttk.Button(btn_frame, text="💾 Esporta", command=lambda: salva_letture_preview(txt, preview_win), style="Verde.TButton").pack(side=tk.LEFT, padx=10)
            ttk.Button(
            btn_frame,
            text="🖨️ Stampa",
            command=lambda: self._stampa_lista_diretta(
            ...
            for giorno_obj in giorni_ordinati:
                ...
                for indice, voce in enumerate(lista_voci): 
                    try:
                        ...
                        if filtro_icona not in ["", "—"] and filtro_icona not in descrizione:
                            continue
                        matches = True
    ...
    def cerca_operazioni(self):
        ...
        def esegui_ricerca(event=None):
            ...
            lbl_totali.config(text=f"✅ Entrate: {tot_entrate_fmt} € | ❌ Uscite: {tot_uscite_fmt} € | 🧮 Saldo: {netto_fmt} €")
            if not risultati:
        ...
        def esporta_risultato():
            ...
            header = f"{'Data':<{W_DATA}} | {'Categoria':<{W_CAT}} | {'Descrizione':<{W_DESC}} | {'Tipo':<{W_TIPO}} | {'Importo (€)':>{W_IMP}}\n"
            ...
            contenuto_preview += f"Entrate Totali: {tot_entrate_fmt} €\n"
            contenuto_preview += f"Uscite Totali:  {tot_uscite_fmt} €\n"
            contenuto_preview += f"Saldo Netto:    {netto_fmt} €\n\n"
            ...
            preview_popup = tk.Toplevel(finestra)
            ...
            frame_btn = ttk.Frame(preview_popup)
    ...
    def anteprima_e_stampa_txt(self):
        ...
        ttk.Button(frame_bottoni, text="🖨️ Stampa", style="Blu.TButton", command=stampa).pack(side="left", padx=20)
    ...
    def gestione_login(self):
        ...
        if ("auto" not in args) and AUTO_ICONIZE_STARTUP:
                ...
                if salvata and hash_pw("") == salvata:
                        print("✅ Login bypassato (Password disattivata + Iconizza all'avvio).")
                        login_riuscito[0] = True
                        self.after(3000, self._iconizza_finestra_startup)
                        return login_riuscito[0]
    ...
    def mostra_report_popup(self, testo):
        ...
        ttk.Button(frame_bottoni, text="❌ Chiudi", style="Giallo.TButton", width=12, command=on_report_close).pack(side="right")
    ...
    def genera_html_consultazione(self, file_selezionato=None):
        ...
        if not inizializza_db_file(percorso_db):
             return f"""<!DOCTYPE html><html><head><title>Errore database</title><meta charset="utf-8"></head><body style='font-family:Arial; background:#fff; padding:20px;'><h2 style='color:#b00;'>❌ Errore database</h2><p style='font-size:1.5em;'>⚠️ Impossibile creare il file <strong>{percorso_db}</strong>.</p><a href='/' style='display:inline-block; margin-top:20px; font-size:1.5em; text-decoration:none; color:#0078D4;'>🔙 Torna alla Home</a></body></html>"""
        try:
            with open(percorso_db, "r", encoding="utf-8") as f:
                contenuto = f.read().strip()
                dati_supermercati = json.loads(contenuto) if contenuto else {}
        except Exception as e:
            return f"""<!DOCTYPE html><html><head><title>Errore JSON</title><meta charset="utf-8"></head><body style='font-family:Arial; background:#fff; padding:20px;'><h2 style='color:#b00;'>❌ Errore JSON</h2><p style='font-size:1.5em;'>Errore lettura database supermercati: {e}</p><a href='/' style='display:inline-block; margin-top:20px; font-size:1.5em; text-decoration:none; color:#0078D4;'>🔙 Torna alla Home</a></body></html>"""
        supermercati = sorted(dati_supermercati.keys())
        page_title = "🔍 Consultazione Cataloghi e Ultima Spesa"
        ...
        <head>
          ...
          <style>
            ...
            /* OTTIMIZZAZIONI PER LE TABELLE DEI CATALOGHI */
            table {{ 
                width: 100%; 
                border-collapse: collapse; 
                font-size: 0.75em; 
                table-layout: fixed; /* Forza la larghezza del 100% per lo schermo */
            }}
            th, td {{ 
                border: 1px solid #ddd; 
                padding: 4px; /* Ridotto il padding da 8px a 4px */
                text-align: left; 
                overflow: hidden; /* Nasconde il testo che eccede */
                text-overflow: ellipsis; /* Aggiunge i puntini di sospensione (utile per le descrizioni) */
            }}
            th {{ 
                background: #0078D4; 
                color: white; 
                text-align: center; 
                white-space: normal; /* Permette all'intestazione di andare a capo */
            }}
            /* FINE OTTIMIZZAZIONI */
            ...
            pre {{ 
                ...
                overflow-x: auto; 
                font-size: 0.85em; /* RIDUZIONE FONT LISTA SPESA */
            }}
          </style>
          <script>
            function toggleMenu() {{
        ...
        <div class='section-title' style='margin-top: 0;' onclick="toggle('{lista_id}')">
            📋 Contenuto ULTIMA LISTA: {file_visualizzato}
            <span id='icon_{lista_id}'>▶</span>
        </div>
        """
        html += f"""
        ...
        if is_raw_content:
            ...
            html += f"""
            <p>❌ <strong>Impossibile visualizzare la lista spesa.</strong></p>
            <p>Assicurati che ci sia almeno un file 'Lista_Spesa_....txt' nella cartella 'export'.</p>
            """
        html += "</div>"
        html += f"<div class='section-title' onclick=\"toggle('cataloghi_sezione')\">🛒 Consultazione Cataloghi ({len(supermercati)}) <span id='icon_cataloghi_sezione'>▶</span></div><div id='cataloghi_sezione' class='section-content' style='display: none;'>"
        ...
        if supermercati:
            for superm in supermercati:
                ...
                if articoli:
                    ...
                    html += "<table><tr><th>Articolo</th><th>Descrizione</th><th>Categoria</th><th>Prezzo Normale (€)</th><th>Promo Attiva/Valore (€)</th></tr>"
            ...
            html += "<p>⚠️ Il database dei cataloghi è vuoto o non contiene supermercati registrati.</p>"
        html += "</div>" 
        html += f"""
            <a href="/" class="back">🏠 Torna alla Home</a>
          </main>
    ...
    def pagina_statistiche_annuali_web(self):
        ...
        try:
            ...
            totale_entrate_match = re.search(r"Entrate Totali:.*?([€\s\d.,-]+)", raw, re.DOTALL)
            totale_uscite_match = re.search(r"Uscite Totali:.*?([€\s\d.,-]+)", raw, re.DOTALL)
            ...
            report_content = "⚠ Report Testuale: Dati non disponibili (Controlla calcola_statistiche_annuali_pura)."
            totale_entrate = None
            totale_uscite = None
        report_content = report_content.replace("🔹 Mese corrente", "<strong><span style='color:#c43b2e;'>🗓️ Mese corrente</span></strong>")
        report_content = report_content.replace("🔹 Da inizio anno", "<strong><span style='color:#d48300;'>📆 Da inizio anno</span></strong>")
        report_content = report_content.replace("🔹 Proiezione fine anno", "<strong><span style='color:#0078D4;'>📊 Proiezione fine anno</span></strong>")
        report_content = report_content.replace("✅ Risparmio previsto", "<strong><span style='color:green;'>💰 Risparmio previsto</span></strong>")
        report_content = report_content.replace("⚠️ Possibile extra spesa", "<strong><span style='color:red;'>📉 Possibile extra spesa</span></strong>")
...
<div style="padding: 10px 20px; font-size: 1.1em; font-weight: bold; background: #fff; border-bottom: 2px solid #0078D4;">
    <p style="margin: 0;">💸 Riepilogo Totale Anno:</p>
    <p style="margin: 5px 0 0 0;">
        💰 Entrate: <span style='color:green;'>{entrate_display}</span> | 
        📉 Uscite: <span style='color:red;'>{uscite_display}</span>
    </p>
</div>
"""
        else:
 ...
 🔹 Proiezione fine anno
  ...
  • Spesa effettiva {anno_prec}: € {stima_anno_prec:,.2f}
    ...
    def spesa_supermercato(self):
        try:
            ...
            if not hasattr(self, '_tkcalendar_warned'):
                print("AVVISO: Libreria 'tkcalendar' non trovata. La data dovrà essere inserita manualmente (gg/mm/aaaa).")
        ...
        def _cerca_articoli_crud(supermercato, treeview, ricerca_var):
            ...
            for articolo in dati_supermercati.get(supermercato, []):
                ...
                if testo_ricerca in nome or testo_ricerca in categoria.lower() or testo_ricerca in descrizione:
        ...
        def _funzione_crud(azione, supermercato, treeview, frame_input):
            ...
            if azione == 'inserisci':
                ...
                if not selezione:
                    self.show_custom_warning("Selezione", "Selezionare **prima** un articolo per modificarlo."); return
        ...
        def _svuota_supermercato(supermercato, treeview):
            ...
            if self.show_custom_askyesno("Azzeramento Dati", f"⚠️ Sei sicuro di voler cancellare TUTTI gli articoli\n dal supermercato '{supermercato}'?"):
        ...
        def _ricarica_lista_spesa(tv_lista):
            ...
            tv_lista.tag_configure('supermarket', font=('Arial', 10, 'bold'), foreground='blue')
        ...
        def _rimuovi_articolo_da_lista(tv_lista):
            ...
            if iid_selezionato.startswith('item_'):
                ...
                if chiave_lista in lista_spesa_data:
                    ...
                    _ricarica_lista_spesa(tv_lista)
                    messaggio_notifica = (
                        f"🗑️ Articolo Rimosso dalla lista.\n"
                ...
                if self.show_custom_askyesno("Conferma", "Sei sicuro di voler rimuovere tutti gli articoli\n di questo supermercato dalla lista?"):
                    ...
                    if chiavi_da_rimuovere:
                        ...
                        _ricarica_lista_spesa(tv_lista)
                        messaggio_notifica = f"🗑️ Gruppo Rimosso dalla lista.\nSupermercato: **{superm}**"
                        show_temporary_notification(tv_lista.winfo_toplevel(), 
                                                    "Conferma Rimozione", 
                                                    messaggio_notifica,
                                                    duration_ms=2500)
        ...
        def _mostra_anteprima_esportazione():
            ...
            preview_popup = tk.Toplevel(popup)
            ...
            text_area.config(state='disabled')
            text_area.pack(fill='both', expand=True, padx=10, pady=10)
            frame_btn = ttk.Frame(preview_popup); frame_btn.pack(pady=(0, 10))
            ...
            ttk.Button(frame_btn, text="🖨️ Stampa", style='Blu.TButton', command=lambda: self._stampa_lista_diretta(anteprima_text, self.show_custom_warning)).pack(side='left', padx=5)
        ...
        def _aggiorna_lista_spesa_intelligente(tv_risultati):
            if tv_risultati:
                ...
                risultati_finali.sort(key=lambda x: x.get('nome', '').strip().lower())
        ...
        def _esegui_rinomina_supermercato(combo, treeview_crud, ricerca_var):
            ...
            if old_superm == "Seleziona Supermercato" or not old_superm:
                self.show_custom_warning("Modifica Supermercato", "Seleziona prima un supermercato da rinominare."); return
            input_popup = tk.Toplevel(popup)
            input_popup.title(f"Rinomina {old_superm}"); input_popup.transient(popup); input_popup.grab_set()
            ttk.Label(input_popup, text="Nuovo nome:").pack(padx=10, pady=(10, 0))
            new_name_var = tk.StringVar(value=old_superm)
            entry = ttk.Entry(input_popup, textvariable=new_name_var, width=30); entry.pack(padx=10, pady=5); entry.focus_set()
            
            def on_confirm(e=None):
                nonlocal SUPERMERCATI
                new_superm = new_name_var.get().strip(); input_popup.destroy()
                if not new_superm or new_superm == old_superm: return
                ...
                if old_superm in dati_supermercati:
                    articoli_da_aggiornare = dati_supermercati.pop(old_superm)
                    dati_supermercati[new_superm] = articoli_da_aggiornare
                    for articolo in dati_supermercati[new_superm]: articolo['supermercato'] = new_superm
                    try: 
                    ...
                    self.show_custom_warning("Successo", f"Supermercato '{old_superm}' rinominato in '{new_superm}'.")
        ...
        def export_supermercati_db():
            ...
            btn_frame = ttk.Frame(input_popup); btn_frame.pack(pady=(0, 10))
            ttk.Button(btn_frame, text="Annulla", command=input_popup.destroy, style='Giallo.TButton').pack(side='left', padx=5)
            ttk.Button(btn_frame, text="Rinomina", command=on_confirm, style='Blu.TButton').pack(side='left', padx=5)
            entry.bind("<Return>", on_confirm)
            input_popup.update_idletasks()
            w = input_popup.winfo_reqwidth(); h = input_popup.winfo_reqheight()
            x = popup.winfo_rootx() + (popup.winfo_width() // 2) - (w // 2)
            y = popup.winfo_rooty() + (popup.winfo_height() // 2) - (h // 2)
            input_popup.geometry(f'+{x}+{y}')
            input_popup.protocol("WM_DELETE_WINDOW", input_popup.destroy)
        ...
        menu_db.add_separator()
        menu_db.add_command(label="❌ Chiudi (ESC)", command=lambda: (self.deiconify(), popup.destroy()))
        ...
        popup.bind("<Escape>", lambda e: (self.deiconify(), popup.destroy()))
        self.iconify()
        ...
        ttk.Button(frame_ricerca_input, text="↺",
                   style='Yellow.TButton', 
    ...
    "Data Inizio Promo", "Data Scadenza Promo", "Prezzo Intero")
        ...
        ttk.Label(frame_lista, text="La Tua Lista Spesa Ottimizzata (Doppio Click su Qtà per Modificare) Promo🔥 Attiva (ROSSO) / Futura (ARANCIO) | Top⭐ Miglior Prezzo Assoluto (VERDE)", font=('Arial', 10, 'bold')
        ...
        ttk.Button(frame_pulsanti_lista, text="❌ Rimuovi Selezionato", style='Rosso.TButton',
                   command=lambda tv=tv_lista_spesa: _rimuovi_articolo_da_lista(tv)).pack(side='left', padx=5)
        ttk.Button(frame_pulsanti_lista, text="🗑️ Svuota Lista", style='Rosso.TButton',
                   command=lambda tv=tv_lista_spesa: _svuota_lista_spesa(tv)).pack(side='left', padx=5)
        ttk.Button(frame_pulsanti_lista, text="🚪 Chiudi", style='Giallo.TButton',
                   command=lambda: on_popup_close()).pack(side='right', padx=5)
        ttk.Button(frame_pulsanti_lista, text="🖨️ Esporta Lista", style='Verde.TButton',
        ...
        ttk.Button(frame_selezione, text="✏️ Rinomina Selezionato", style='Giallo.TButton',
        ...
        canvas_input = tk.Canvas(frame_gestione, height=140)
        canvas_input.grid(row=1, column=0, sticky='ew', pady=5)
        ...
        frame_input = ttk.Frame(canvas_input)
        frame_input.grid_columnconfigure(1, weight=1); frame_input.grid_columnconfigure(3, weight=1)
        canvas_input.create_window((0, 0), window=frame_input, anchor="nw")
        frame_input.bind("<Configure>", lambda e: canvas_input.configure(scrollregion=canvas_input.bbox("all")))
        canvas_input.config(height=170)
        ttk.Label(frame_input, text="Nome Articolo:").grid(row=0, column=0, padx=5, pady=2, sticky='w')
        ttk.Entry(frame_input, textvariable=refs_crud['nome']).grid(row=0, column=1, padx=5, pady=2, sticky='ew')
        ...
        combo_categoria.grid(row=0, column=3, padx=5, pady=2, sticky='ew')
        if CATEGORIE_PREDEFINITE:
        ...
        ttk.Entry(frame_input, textvariable=refs_crud['descrizione']).grid(row=1, column=1, padx=5, pady=2, sticky='ew')
        ttk.Label(frame_input, text="Quantità (es: 500g):").grid(row=1, column=2, padx=5, pady=2, sticky='w')
        ttk.Entry(frame_input, textvariable=refs_crud['quantita']).grid(row=1, column=3, padx=5, pady=2, sticky='ew')
        ttk.Label(frame_input, text="Prezzo Normale (€):").grid(row=2, column=0, padx=5, pady=2, sticky='w')
        ttk.Entry(frame_input, textvariable=refs_crud['prezzo']).grid(row=2, column=1, padx=5, pady=2, sticky='ew')
        ttk.Label(frame_input, text="Prezzo Promo (€):").grid(row=2, column=2, padx=5, pady=2, sticky='w')
        ttk.Entry(frame_input, textvariable=refs_crud['prezzo_promo']).grid(row=2, column=3, padx=5, pady=2, sticky='ew')
        ...
        frame_data_inizio_promo.grid(row=3, column=3, padx=5, pady=2, sticky='ew')
        frame_data_inizio_promo.grid_columnconfigure(0, weight=1)
        entry_data_inizio_promo = ttk.Entry(frame_data_inizio_promo, textvariable=refs_crud['data_inizio_promo'])
        entry_data_inizio_promo.grid(row=0, column=0, sticky='ew')
        ttk.Button(
        ...
        frame_data_ins.grid(row=4, column=1, padx=5, pady=2, sticky='ew')
        frame_data_ins.grid_columnconfigure(0, weight=1)
        entry_data_ins = ttk.Entry(frame_data_ins, textvariable=refs_crud['data_inserimento_prezzo'])
        entry_data_ins.grid(row=0, column=0, sticky='ew')
        ttk.Button(
            frame_data_ins,
            text="🗓️",
            style='Giallo.TButton',
        ...
        frame_data_input.grid(row=4, column=3, padx=5, pady=2, sticky='ew')
        frame_data_input.grid_columnconfigure(0, weight=1)
        entry_data_scadenza = ttk.Entry(frame_data_input, textvariable=refs_crud['data_scadenza'])
        entry_data_scadenza.grid(row=0, column=0, sticky='ew')
        ttk.Button(
            frame_data_input, 
            text="🗓️", 
            style='Giallo.TButton',
        ...
        cols_super = ("Nome", "Descrizione", "Categoria", "Prezzo", "Data Ins.", "Promo", "P. Promo", "Quantità", "Inizio Promo", "Scadenza") # ⬅️ INVERSIONE QUI
        ...
        ttk.Button(frame_pulsanti_crud, text="➡️ Sposta", style='Giallo.TButton',
        ...
        def _genera_testo_esportazione_supermercato(supermercato):
            ...
            WIDTH_SCAD = 10
            intestazione = (
                f"{'Nome':<{WIDTH_NOME}} {'Descrizione':<{WIDTH_DESC}} {'Categoria':<{WIDTH_CAT}} "
                f"{'Qtà':<{WIDTH_QTA}} {'P. Normale':>{WIDTH_PREZZO}} {'P. Promo':>{WIDTH_PROMO}} "
                f"{'In Promo':<{8}} {'Scadenza':<{WIDTH_SCAD}}"
            )
            ...
            for articolo in articoli:
                ...
                data_scadenza = articolo.get("data_scadenza", "")
                prezzo_fmt = f"{float(prezzo):.2f}" if prezzo and prezzo.replace('.', '', 1).isdigit() else ""
                ...
                linea = (
                    ...
                    f"{prezzo_fmt:>{WIDTH_PREZZO}} "
                    f"{promo_fmt:>{WIDTH_PROMO}} "
                    f"{promo:<{8}} "
                    f"{data_scadenza:<{WIDTH_SCAD}}\n"
                )
                testo += linea
            testo += separatore + "\n"
            return testo
        ...
        def _mostra_anteprima_esportazione_supermercato(supermercato):
            ...
            def _esporta_su_file_super(content_text, default_name, preview_popup):
                ...
                if f:
                    try:
                        with open(f, 'w', encoding='utf-8') as file_handle: file_handle.write(content_text)
                        self.show_custom_warning("Successo", f"✅ Catalogo salvato con successo in:\n{f}")
                    except Exception as e:
                        self.show_custom_warning("Errore", f"❌ Impossibile salvare il file:\n{e}")
            preview_popup = tk.Toplevel(popup)
        ...
        ttk.Button(frame_pulsanti_crud, text="🛒 Importa Scontrini", 
...
"""
        testo_stampa_completo = (
            ...
            "C. Funzionalità Menu\n"
            "* Importa Supermercati: Carica archivio ZIP. ATTENZIONE: Sovrascrive TUTTI i dati esistenti.\n"
            "* Esporta Supermercati: Crea un archivio ZIP con l'intero database di backup."
        )
        ...
        help_popup = tk.Toplevel(parent_window) 
    ...
    def gestisci_archivi_pdf(self, categoria_iniziale=None):
        ...
        def trace_importo(name, index, mode):
            ...
            formattato = format_importo(pulito)
        ...
        def open_pdf(event, treeview):
             try:
                 ...
                 if not file_name or file_name == "N/D":
                     self.show_custom_warning("Attenzione", "Il nome del file non è disponibile per l'apertura.")
                     return
                 file_path = os.path.join(DOC_DIR, file_name)
                 if os.path.exists(file_path):
                     webbrowser.open(file_path)
                 else:
                     self.show_custom_warning("File Non Trovato", f"Il file:\n{file_name}\nnon esiste più nel percorso:\n{DOC_DIR}")
        ...
        def load_documents(treeview, filtri_attuali=None):
                ...
                lbl_totali.config(text=f"Totale Filtrato: {totale_formattato} €", fg="blue")
                if num_risultati == 0: 
                        lbl_risultati.config(text="❌ Nessuna corrispondenza per i filtri attuali.", fg="red")
                elif parola_chiave_lower or filtri_attuali: 
                        lbl_risultati.config(text=f"🔍 Trovati {num_risultati} documenti filtrati.", fg="black")
                else: 
                        lbl_risultati.config(text=f"📋 Totale documenti archiviati: {num_risultati}.", fg="gray")
                if num_risultati == 0: lbl_risultati.config(text="❌ Nessuna corrispondenza per i filtri attuali.", fg="red")
                elif parola_chiave_lower or filtri_attuali: lbl_risultati.config(text=f"🔍 Trovati {num_risultati} documenti.", fg="black")
                else: lbl_risultati.config(text=f"📋 Totale documenti archiviati: {num_risultati}.", fg="gray")
                totale_in_centesimi = int(round(totale_filtrato * 100))
                totale_formattato = formatta_importo_pulito(str(totale_in_centesimi))
                lbl_totali.config(text=f"Totale Filtrato: {totale_formattato} €", fg="blue")
        ...
        def inserisci_documento_e_copia():
                ...
                if int(importo_pulito_per_file_raw) < 100:
                        self.show_custom_warning("Attenzione", "Importi inferiori a 1,00 € non sono ammessi.")
                        return
                if int(importo_pulito_per_file_raw) > 100000000:
                        self.show_custom_warning("Attenzione", "Importi superiori a 1.000.000 € non sono ammessi.")
                        return
                ...
                try:
                        if os.path.exists(final_path):
                                messaggio_preliminare = f"ATTENZIONE: Il file '{file_name_format}' è stato sovrascritto!\n\n"
                        ...
                        importo_visualizzato = f"{importo_formattato} €"
        ...
        def cancella_documento():
            ...
            if not selected_items:
                self.show_custom_warning("Selezione", "Seleziona una o più voci da cancellare.")
                return
            try:
        ...
        def mostra_calendario_popup_interno(entry_widget, var_data):
            try:
                bloccante = entry_widget.winfo_toplevel()
            except:
                bloccante = pdf_window 
            if hasattr(self, "popup_calendario") and self.popup_calendario and self.popup_calendario.winfo_exists():
                try: 
                    self.popup_calendario.destroy()
                    self.popup_calendario = None
                    ...
                    pass
                return
            self.popup_calendario = tk.Toplevel(bloccante)
            self.popup_calendario.title("Seleziona una Data")
            self.popup_calendario.transient(bloccante)
            self.popup_calendario.withdraw() 
            def seleziona_data(event=None):
                try:
                    date_val = cal.selection_get()
                    if date_val:
                        var_data.set(date_val.strftime("%d-%m-%Y")) 
                except Exception:
                    pass
                self.popup_calendario.destroy()
            cal = Calendar(
            ...
            try:
                ...
                pass
            cal.bind("<<CalendarSelected>>", seleziona_data)
            cal.bind("<Double-1>", seleziona_data) 
            self.popup_calendario.bind("<Escape>", lambda e: self.popup_calendario.destroy())
            entry_widget.update_idletasks()
            ex = entry_widget.winfo_rootx()
            ey = entry_widget.winfo_rooty()
            eh = entry_widget.winfo_height()
            self.popup_calendario.geometry("350x180")
            self.popup_calendario.update_idletasks() 
            pw = self.popup_calendario.winfo_width()
            ph = self.popup_calendario.winfo_height()
            MIN_WIDTH = 300 
            pw = max(pw, MIN_WIDTH)
            SHIFT_RIGHT = 50
            x = ex + (entry_widget.winfo_width() // 2) - (pw // 2) + SHIFT_RIGHT
            y = ey + eh + 2
            screen_w = self.popup_calendario.winfo_screenwidth()
            screen_h = self.popup_calendario.winfo_screenheight()
            if x + pw > screen_w: x = max(2, screen_w - pw - 2)
            if y + ph > screen_h:
                alt_y = ey - ph - 2
                y = alt_y if alt_y > 0 else max(2, screen_h - ph - 2)
            try:
                self.popup_calendario.geometry(f"{pw}x{ph}+{x}+{y}")
            except Exception:
                self.popup_calendario.geometry(f"350x180+{x}+{y}")
            self.popup_calendario.deiconify()
            self.popup_calendario.focus_set() 
            cal.focus_set()
            bloccante.wait_window(self.popup_calendario) 
            self.popup_calendario = None
        def apri_filtri_avanzati():
            root = pdf_window 
            filtro_win = tk.Toplevel(root)
            filtro_win.title("⚙️ Filtri Avanzati")
            filtro_win.geometry("500x380")
            ...
            combo_cat_filtro = ttk.Combobox(frame_filtri, textvariable=categoria_var, values=valori_categoria, state='readonly', width=27)
            ...
            ttk.Combobox(frame_filtri, textvariable=tipo_var, values=valori_tipo, state='readonly', width=27).grid(row=2, column=1, columnspan=2, padx=5, pady=5, sticky="ew")
            ...
            ttk.Button(frame_filtri, text="🗓️", style='Giallo.TButton', 
                       command=lambda: mostra_calendario_popup_interno(entry_data_da, data_da_var), width=3).grid(row=4, column=2, padx=(0, 0), pady=5, sticky="w")
            ...
            ttk.Button(frame_filtri, text="🗓️", style='Giallo.TButton', 
                       command=lambda: mostra_calendario_popup_interno(entry_data_a, data_a_var), width=3).grid(row=5, column=2, padx=(0, 0), pady=5, sticky="w")
            ...
            frame_btns = ttk.Frame(filtro_win); frame_btns.pack(pady=10)
        ...
        def chiudi_finestra():
             if hasattr(self, 'popup_calendario') and self.popup_calendario and self.popup_calendario.winfo_exists(): self.popup_calendario.destroy()
             self.filtri_avanzati = {}
        ...
        menu_archivio.add_command(label="❌ Esporta documenti", command=self.esegui_export_documenti_pdf)
        menu_archivio.add_command(label="❌ Importa documenti", command=self.esegui_import_documenti_pdf)
        menu_archivio.add_separator()
        menu_archivio.add_command(label="❌ Chiudi (ESC)", command=chiudi_finestra)
 
        self.pdf_window = pdf_window
        pdf_window.title("📂 Archivio Documenti PDF")
        pdf_window.transient(self); pdf_window.geometry("1200x650"); pdf_window.minsize(width=800, height=500)
        pdf_window.bind('<Escape>', lambda e: chiudi_finestra())
        frame_input = ttk.Frame(pdf_window, padding="10"); frame_input.pack(fill='x', padx=10, pady=(10, 5))
        data_var = tk.StringVar(value=datetime.now().strftime("%d-%m-%Y")); importo_var = tk.StringVar()
        importo_var.trace_add("write", trace_importo)
        ttk.Label(frame_input, text="Data:").grid(row=0, column=0, padx=5, pady=5, sticky="w")
        sub_frame_data = ttk.Frame(frame_input); sub_frame_data.grid(row=0, column=1, padx=0, pady=5, sticky="w")
        entry_data = ttk.Entry(sub_frame_data, textvariable=data_var, width=12); entry_data.pack(side="left", padx=0)
        ttk.Button(sub_frame_data, text="🗓️", style="Giallo.TButton", command=lambda: mostra_calendario_popup_interno(entry_data, data_var), width=2).pack(side="left", padx=0)
        ttk.Label(frame_input, text="Categoria:").grid(row=0, column=3, padx=5, pady=5, sticky="w")
        combo_categoria = ttk.Combobox(frame_input, values=(getattr(self, 'categorie', categorie_vuote)), width=35, state='readonly') 
        ...
        entry_descrizione = ttk.Entry(frame_input, width=35); entry_descrizione.grid(row=0, column=6, padx=5, pady=5, sticky="w")
        ...
        combo_tipo = ttk.Combobox(frame_input, values=tipi_movimento, width=15, state='readonly')
        combo_tipo.set("Uscita")
        combo_tipo.grid(row=1, column=4, padx=5, pady=5, sticky="w")
        frame_ricerca = ttk.Frame(pdf_window, padding="10 0 10 5"); frame_ricerca.pack(fill='x', padx=10)
        ...
        ttk.Button(frame_ricerca, text="⚙️ Filtri Avanzati", command=apri_filtri_avanzati, style="Blu.TButton").pack(side="right", padx=5)
        ttk.Button(frame_ricerca, text="↺ ", command=resetta_campo, style="Yellow.TButton").pack(side="right")
        cols = ("data", "categoria", "descrizione", "importo", "tipo", "nome_completo")
        tree = ttk.Treeview(pdf_window, columns=cols, show='headings', selectmode='extended')
        ...
        frame_bottom_buttons = ttk.Frame(pdf_window, padding="10"); frame_bottom_buttons.pack(fill='x', padx=10, pady=(5, 10))
        ttk.Button(frame_bottom_buttons, text="📥 Archivia PDF", style='Verde.TButton', command=inserisci_documento_e_copia).pack(side='left', padx=10)
        ttk.Button(frame_bottom_buttons, text="🗑️ Cancella Documenti", style='Rosso.TButton', command=cancella_documento).pack(side='left', padx=10)
        ttk.Button(frame_bottom_buttons, text="🖨️ Stampa Selezionati", style='Blu.TButton', command=stampa_documenti_selezionati).pack(side='left', padx=10)
        ttk.Button(frame_bottom_buttons, text="❌ Chiudi", command=chiudi_finestra, style='Giallo.TButton').pack(side='right', padx=10)
        filtra_documenti() 
    ...
    def mostra_help_pdf(self):
        ...
        tk.Label(
            ...
            text="• **Apertura Documento (Doppio Clic):** Doppio clic su una riga per aprire il file PDF associato.\n"
                 ...
                 "• **Cancellazione (Canc/Bottone):** Seleziona una riga e premi **CANC** (o usa il bottone '🗑️ Cancella Documento') per eliminare file e registro.",
            font=("Arial", 9),
            justify=tk.LEFT,
            anchor='w',
            wraplength=900
    ...
    def check_supermarket_update(self):
        ...
        try:
            ...
            print("⚠️ Errore nel parsing dell'URL GITHUB_SUPERMARKET.")
            return
        except NameError:
            print("⚠️ Errore: Costanti GITHUB_SUPERMARKET o NOME_EDITOR_LOCALE non definite.")
            return
        try:
            api_url = f"https://api.github.com/repos/{REPO_OWNER}/{REPO_NAME}/commits"
            params = {"path": nome_file_locale, "per_page": 5} 
            ...
            if remote_time.date() > local_time.date():
                ...
                msg = (
                    "🆕 È stato rilevato un possibile aggiornamento per l'Editor.\n\n"
            ...
            print(f"🌐 Connessione assente o GitHub non raggiungibile per {nome_file_locale}.")
        except requests.exceptions.RequestException as e:
            print(f"⚠️ Errore HTTP o API per {nome_file_locale}: {e}")
        except Exception as e:
            print(f"⚠️ Errore generico durante il controllo aggiornamento per {nome_file_locale}: {e}")
        ...
        try:
            ...
            if remote_time.date() > local_time.date():
                ...
                def aggiorna():
                    ...
                    win.destroy()
                    if self.aggiorna(GITHUB_SUPERMARKET, nome_file_locale):
                        # Aggiunta logica per chiusura popup superiore e riavvio se necessario
                        self.show_custom_info("Editor Aggiornato", f"L'editor {nome_file_locale} è stato aggiornato!")
        ...
        try:
            ...
            if os.path.exists(nome_backup):
                try:
                    ...
                    print(f"⚠️ Attenzione: Impossibile rimuovere il file di backup '{nome_backup}': {e}")
            if rimossi:
            ...
            self.show_custom_warning(
                ...
                "⚠️ La costante NOME_EDITOR_LOCALE non è definita. Impossibile procedere."
            )
        except Exception as e:
    ...
    def mostra_avviso_iconizzata(self):
        ...
        splash.bind("<Motion>", lambda e: self._reset_inattivita())
        splash.bind("<Button>", lambda e: self._reset_inattivita())
        splash.bind("<Key>", lambda e: self._reset_inattivita())
        label.bind("<Motion>", lambda e: self._reset_inattivita())
        label.bind("<Button>", lambda e: self._reset_inattivita())
        label.bind("<Key>", lambda e: self._reset_inattivita())
                ...
                def aggiorna():
                    ...
                    try:
                        ...
                        if 'shutil' in sys.modules and os.path.exists(nome_backup):
                            shutil.copy2(nome_backup, NOME_FILE)
                            os.remove(nome_backup)
                        self.show_custom_warning("Attenzione", f"❌ Aggiornamento fallito durante il download/riavvio:\n{e}")
                        return
    ...
    def forza_check_aggiornamento_con_api(self):
        ...
        if os.path.exists(RIMANDA_FILE):
            try:
                ...
                print(f"⚠️ Errore durante la cancellazione del file rimando: {e}")
        try:
            api_url = f"https://api.github.com/repos/{REPO_OWNER}/{REPO_NAME}/commits"
            params = {"path": NOME_FILE, "per_page": 1}
            response = requests.get(api_url, params=params, timeout=5)
            ...
            commits = response.json()
            if not commits:
                self.show_custom_warning("Controllo Aggiornamento", "⚠️ Nessun commit trovato. Impossibile verificare lo stato.")
                return 
            commit_date = commits[0]["commit"]["committer"]["date"]
            remote_time = datetime.strptime(commit_date, "%Y-%m-%dT%H:%M:%SZ").replace(microsecond=0)
            changelog_text = ""
            ...
            if remote_time.date() > local_time.date():
                ...
                self.show_custom_warning("Controllo Manuale", "✅ Nessun nuovo aggiornamento software disponibile. Sei già all'ultima versione.")
                return
        except ConnectionError:
            print("🌐 Connessione assente o GitHub non raggiungibile.")
        except RequestException as e:
    ...
    def ripristina_da_backup(self):
        ...
        if not os.path.exists(nome_backup):
            self.show_custom_info("Ripristino Non Riuscito", 
                                  f"❌ File di backup ({nome_backup}) non trovato. Impossibile procedere al ripristino.")
            return
        conferma = self.show_custom_askyesno(
            title="Ripristino",
            message=f"Sei sicuro di voler ripristinare il file '{NOME_FILE}'\ndalla copia di backup?\n\n"
                    f"Questo annullerà l'ultima modifica/aggiornamento\ne riavvierà l'applicazione."
        )
        if not conferma:
            self.show_custom_warning("Ripristino Annullato", "Ripristino annullato dall'utente.")
            return
        try:
            shutil.copy2(nome_backup, NOME_FILE)
            print(f"✅ Ripristino completato: {NOME_FILE} ripristinato da {nome_backup}.")
            os.remove(nome_backup)
            self.show_custom_warning("Ripristino Eseguito", "✅ Ripristino completato. Riavvio in corso...")
    ...
    def visualizza_changelog(self):
        ...
        try:
            ...
            commits = response.json()
            if not commits:
                self.show_custom_info("Changelog Vuoto", "⚠️ Nessuno storico di commit trovato per questo file.")
                return
            for commit in commits:
                date_str = commit["commit"]["committer"]["date"]
                commit_dt = datetime.strptime(date_str, "%Y-%m-%dT%H:%M:%SZ")
            ...
            win.bind('<Escape>', lambda e: win.destroy())
            ttk.Label(win, text="Storico degli ultimi commit da GitHub:", font=("Arial", 10, "bold")).pack(pady=(10, 0))
            frame_changelog = ttk.Frame(win)
            frame_changelog.pack(padx=20, pady=(5, 10), fill='both', expand=True)
            scrollbar = ttk.Scrollbar(frame_changelog)
            scrollbar.pack(side=tk.RIGHT, fill=tk.Y)
            ...
            text_area.pack(side=tk.LEFT, fill='both', expand=True)
            scrollbar.config(command=text_area.yview)
            ttk.Button(win, text="❌ Chiudi", command=win.destroy, style="Giallo.TButton").pack(pady=(0, 10))
            win.withdraw()
            win.update_idletasks()
            min_w, min_h = 660, 410
            w = max(win.winfo_width(), min_w)
            ...
            self.show_custom_warning("Errore Connessione", f"⚠️ Impossibile connettersi a GitHub:\n{e}")
        except Exception as e:
            self.show_custom_warning("Errore", f"❌ Errore generico durante la visualizzazione dello storico:\n{e}")
                  
    def esegui_backup_zip(self):
        from datetime import datetime
        percorso_archivio = ""
        cartella_temp_path = None
        print(f"Tentativo di backup di '{self.current_folder}'...")
        try:
            ...
            if not percorso_completo_output:
                print("❌ Backup annullato dall'utente.")
                return ""
            self.update_idletasks()
            self.update() 
            percorso_output_senza_ext, _ = os.path.splitext(percorso_completo_output)
            ...
            print("✅ Copia temporanea creata (nel percorso TEMP di sistema).")
            ...
            print(f"✅ Backup di '{self.current_folder}' completato.")
            self.show_custom_warning ("Salvataggio", f"Backup di {self.current_folder} completato. !")
            print(f"File salvato in: {percorso_archivio}")
        except Exception as e:
            print(f"❌ ERRORE FATALE durante il backup: {e}")
        finally:
            if cartella_temp_path and os.path.exists(cartella_temp_path):
                shutil.rmtree(cartella_temp_path, ignore_errors=True)
                print("✅ Pulizia cartella TEMP completata.")
        return percorso_archivio 
    ...
    def gestisci_configurazione(self):
        ...
        def reset_defaults():
            ...
            update_timeout_label(default_timeout_min)
            ...
            update_anni_label(DEFAULT_CONFIG["anni_da_mantenere"])
            self.show_custom_info("Reset", "Impostazioni ripristinate ai valori predefiniti. Clicca su Salva per applicare.")
        def chiudi_config():
        ...
        def salva_e_chiudi():
            try:
                ...
                if nuova_config["webserver_port"] < 1024 or nuova_config["webserver_port"] > 65535:
                ...
                riavvia_subito = self.show_custom_askyesno(
                    ...
                    message="Per applicare completamente alcune modifiche\n(es. porta WebServer, timeout, pulizia database)\nè necessario riavviare l'applicazione.\n\nRiavviare ora?"
                )
                if riavvia_subito:
                    riavvia_app_definitivo()
                else:
                    self.show_custom_warning("Riavvio in Sospeso", "⚠️ Riavvio posticipato. Le modifiche saranno applicate al prossimo avvio manuale.")
        ...
        def create_entry(parent, label_text, variable, row):
            label = ttk.Label(parent, text=label_text)
            label.grid(row=row, column=0, sticky="w", pady=5)
            entry = ttk.Entry(parent, textvariable=variable, width=15)
            entry.grid(row=row, column=1, sticky="w", pady=5, padx=(0, 5))
            
        create_checkbutton(main_frame, "Abilita Flusso Login/Iconizza Automatica:", self.var_auto_login, 0)
        create_checkbutton(main_frame, "Abilita Web Server all'avvio:", self.var_webserver_enabled, 1)
        create_entry(main_frame, "Porta Web Server (8080 default):", self.var_port, 2)
        create_checkbutton(main_frame, "Carica Posizione Finestra Salvata:", self.var_load_geometry, 3)
        create_checkbutton(main_frame, "Iconizza App in caso di Inattività:", self.var_iconizza_inattivita, 4)
        ttk.Label(main_frame, text="Timeout Inattività Iconizza (5-60 minuti):").grid(row=5, column=0, sticky="w", pady=(10, 0))
        timeout_label = ttk.Label(main_frame, text="")
        timeout_label.grid(row=5, column=1, sticky="w", columnspan=2, pady=(10, 0))
        timeout_scale = ttk.Scale(
            ...
            orient="horizontal",
            length=250,
            variable=self.var_timeout_minuti,
            command=update_timeout_label
        )
        timeout_scale.grid(row=6, column=0, columnspan=3, pady=(0, 10), sticky="ew")
        update_timeout_label(self.var_timeout_minuti.get())
        ttk.Label(main_frame, text="Anni di Storico Dati (periodo di conservazione - 2-15):  ").grid(row=7, column=0, sticky="w", pady=(10, 0))
        anni_label = ttk.Label(main_frame, text="")
        anni_label.grid(row=7, column=1, sticky="w", columnspan=2, pady=(10, 0))
        anni_scale = ttk.Scale(
            main_frame,
            from_=2,
            to=15,
            orient="horizontal",
            length=250,
            variable=self.var_anni_da_mantenere,
            command=update_anni_label
        )
        anni_scale.grid(row=8, column=0, columnspan=3, pady=(0, 10), sticky="ew")
        ...
        button_frame.grid(row=10, column=0, columnspan=3, pady=10, sticky="ew")
        button_frame.columnconfigure(0, weight=1)
        button_frame.columnconfigure(1, weight=1)
        button_frame.columnconfigure(2, weight=1)
        save_button = ttk.Button(button_frame, text="💾 Salva Configurazione", command=salva_e_chiudi, style="Verde.TButton")
        save_button.grid(row=0, column=0, padx=(0, 10), sticky="e")
        defaults_button = ttk.Button(button_frame, text="↩️ Defaults", command=reset_defaults, style="Rosso.TButton")
        defaults_button.grid(row=0, column=1, padx=5)
        close_button = ttk.Button(button_frame, text="❌ Chiudi", command=chiudi_config, style="Giallo.TButton")
        close_button.grid(row=0, column=2, padx=(10, 0), sticky="w")
        config_window.withdraw()
        config_window.minsize(500, 380)
...
def backup_incrementale(file_path, cartella_backup="backup", max_backup=10):
    ...
    backup_file = os.path.join(cartella_backup, f"{data}-{nome}")
...
def install_genai():
    try:
        ...
        print("Il modulo 'google-genai' non è stato trovato. Installazione in corso...")
        try:
...
def leggi_configurazione_globale():
    ...
    if not os.path.exists(CONFIG_FILE):
        try:
            ...
            print("💾 Creato file config.json di default.")
            return final_config
        except Exception:
            return final_config
    try:
        ...
        print("⚠️ Errore di lettura config.json. Uso i default.")
    return final_config
