...
class GestioneSpese(tk.Tk):
    ...
    def mostra_transazioni_popup_2(self, data_filter, title):
        anno = data_filter.get("anno")
        mese = data_filter.get("mese")
        categoria = data_filter.get("categoria") 
        tipo = data_filter.get("tipo")
        ...
        for data, voci in self.spese.items():
            if anno and anno != "Tutti":
                if str(data.year) != anno: 
                    continue
            if mese and mese != "Tutti":
                if data.month != mese:
                    continue
            for entry in voci:
                try:
                    cat, desc, imp, entry_tipo = entry[:4]
                    cat_normalized = cat.strip().title() # Normalizza 'ACQUA SPA' in 'Acqua Spa'
                    entry_imp = float(imp)
                    entry_tipo = entry_tipo.strip()
                except Exception:
                    continue
                if categoria and cat_normalized != categoria: 
                    continue
                if tipo and entry_tipo != tipo:
                    continue
    ...
    def calcola_mancanti(self):
        ...
        def on_tree_select(event):
            ...
            self.mostra_transazioni_popup_2(data_filter, title)
