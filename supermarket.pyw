...
ICON_PATH = os.path.join(base_dir, "db", "casa-facile.png")
...
class ScontrinoParserApp:
    ...
    def _carica_db(self):
        file = filedialog.askopenfilename(
            title="Scegli File Database Completo (.json)",
            defaultextension=".json",
            filetypes=[("File JSON", "*.json")]
        )
        if file:
            dati_caricati = self._carica_db_da_file(file)
            if dati_caricati is not None:
                self.dati_db_piatti = dati_caricati
                self.db_file_path = file
                self._ricarica_treeview(self.tree_db, self.dati_db_piatti)
                self.db_path_label.config(text=f"DB: {os.path.basename(file)}")
                self._log(f"✅ Caricato Database Completo da {os.path.basename(file)} ({len(dati_caricati)} articoli).")
    ...
    def _salva_db(self):
        ...
        if not filepath:
            filepath = filedialog.asksaveasfilename(
                defaultextension=".json",
                filetypes=[("File JSON", "*.json")],
                initialfile="database_completo.json",
                title="Salva Database Completo"
            )
        if filepath:
...
if __name__ == "__main__":
    ...
    custom_button_font = ('Arial', 9) 
    custom_treeview_font = ('Arial', 8, 'normal') 
    style.configure("Custom.Treeview.Heading", font=('Arial', 8, 'normal')) 
    style.configure("Custom.Treeview", font=custom_treeview_font, rowheight=18) 
    style.map('Custom.Treeview', 
              background=[('selected', '#5c99ff')],
              foreground=[('selected', 'white')])
    style.configure('Custom.TButton', background='#3f51b5', foreground='white', font=custom_button_font, padding=5, bordercolor='#3f51b5', relief='flat')
    style.map('Custom.TButton', background=[('active', '#303f9f')])
    style.configure('Accent.TButton', background='#4CAF50', foreground='white', font=custom_button_font, padding=5, bordercolor='#4CAF50', relief='flat')
